...
function lib.client:startProgress(object, async)
    ...
    if object?.prop ~= nil then
        ...
        if objectName ~= '' and targetBone ~= 0 and targetCoords and targetRotation then
            ...
            AttachEntityToEntity(prop, ped, boneIndex, targetCoords.x, targetCoords.y, targetCoords.z, targetRotation.x, targetRotation.y, targetRotation.z, true, true, false, true, 1, true)
        end
    end
